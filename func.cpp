...
void PrintCircle(circle* c) {
    std::cout << c->cntr.x << " " << c->cntr.y << std::endl;
...
bool InShapeCircle(circle* c, float x, float y) {
    return (x - c->cntr.x) * (x - c->cntr.x) + (y - c->cntr.y) * (y - c->cntr.y) <= c->rad * c->rad;
}
...
bool OnContourCircle(circle* c, float x, float y) {
    float distance_sq = (x - c->cntr.x) * (x - c->cntr.x) + (y - c->cntr.y) * (y - c->cntr.y);
    float rad_sq = c->rad * c->rad;
    // Сравнение с допуском для вещественных чисел
    return std::abs(distance_sq - rad_sq) < 0.0001f;
}
